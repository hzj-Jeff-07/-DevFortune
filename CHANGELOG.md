...
- **Web JSON API**：`GET /api/fortune?date=&time=&locale=`，带参数校验、明确的 400 错误信息与缓存策略（指定日期长缓存，默认当日不缓存）/ Web JSON API with validation and cache headers
...
- **评分曲线修正**：日主强度分段函数在占比 0.1 与 0.5 处不连续、0.5 之后越失衡分数反而越高，改为单调分段线性并加单调性回归测试 / Day-master scoring curve is now monotonic piecewise-linear (the old one had discontinuities at 0.1/0.5 and rewarded imbalance past 0.5)
- **CLI 选项缺值**：`-d`/`-t`/`-f`/`-l` 缺少参数值时告警，不再吞掉下一个选项 / CLI options missing their value warn instead of swallowing the next flag
